...
int QSPI_FLASH::set_dummy_cycles_and_strength(
  u8 dummy_and_strenght,
  u8 dummy_and_strength_mask,
  u32 read_config_command,
  u32 write_config_command) {
  int result = 0;
  u8 status_cnfg[2];
  /*read first*/
  ...
  write_reg(
    ...
    &status_cnfg[0],
    QSPI_FLAG_IS_REGISTER_WIDTH_16);
  u16 i = 10000;
  while (i--) {
  };
...
u16 QSPI_FLASH::read_flash(u32 address, u8 *buff, u16 length) {
  ...
  if (length > mergin) {
    ...
    page_count
      = length / (u16)m_page_size; // calculate number of pages to be written
    remain = length % m_page_size; // calculate the remainder after filling up
                                   // one or more entire pages
    while (page_count--) {
