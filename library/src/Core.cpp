...
int Core::set_pin_function(const core_pinfunc_t &req) {
  return ioctl(IoRequest(I_CORE_SETPINFUNC), IoConstArgument(&req));
}

int Core::get_mcu_board_config(mcu_board_config_t &config) {
  return ioctl(IoRequest(I_CORE_GETMCUBOARDCONFIG), IoArgument(&config));
}

int Core::set_clock_divide(u32 value) {
  return ioctl(IoRequest(I_CORE_SETCLKDIVIDE), IoIntArgument(value));
}
...
CoreInfo Core::get_info() {
  ...
  core_info_t info;
  if (core.ioctl(IoRequest(I_CORE_GETINFO), IoArgument(&info)) < 0) {
    return CoreInfo();
  }
